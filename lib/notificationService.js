import { collection, addDoc, serverTimestamp, doc, deleteDoc, updateDoc, getDocs, query, where, getDoc, setDoc } from 'firebase/firestore';
...
export async function deleteNotification(notificationId) {
  ...
  if (notification.userId) {
    try {
      const userNotifRef = doc(db, 'users', notification.userId, 'notifications', notificationId);
      await deleteDoc(userNotifRef);
    } catch (err) {
      console.warn('Failed to delete user subcollection notification:', err);
    }
  }
}
...
export async function markNotificationRead(notificationId) {
  ...
  if (notification.userId) {
    try {
      const userNotifRef = doc(db, 'users', notification.userId, 'notifications', notificationId);
      await updateDoc(userNotifRef, { isRead: true });
    } catch (err) {
      console.warn('Failed to mark user subcollection notification read:', err);
    }
  }
}
...
export async function markAllNotificationsRead(userId) {
  ...
  querySnapshot.forEach((docItem) => {
    ...
    const userIdValue = docItem.data().userId;
    if (userIdValue) {
      const userRef = doc(db, 'users', userIdValue, 'notifications', docItem.id);
      writes.push(updateDoc(userRef, { isRead: true }).catch((err) => {
        console.warn('Failed to mark user subcollection notification read:', err);
      }));
    }
  });
